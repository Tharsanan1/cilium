...
type SecurityPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*v2.SecurityPolicy, err error)
...
// List lists all SecurityPolicies in the indexer.
func (s *securityPolicyLister) List(selector labels.Selector) (ret []*v2.SecurityPolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2.SecurityPolicy))
	})
	return ret, err
}
...
type SecurityPolicyNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2.SecurityPolicy, error)
...
// List lists all SecurityPolicies in the indexer for a given namespace.
func (s securityPolicyNamespaceLister) List(selector labels.Selector) (ret []*v2.SecurityPolicy, err error) {
...
// Get retrieves the SecurityPolicy from the indexer for a given namespace and name.
func (s securityPolicyNamespaceLister) Get(name string) (*v2.SecurityPolicy, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2.Resource("securitypolicy"), name)
	}
	return obj.(*v2.SecurityPolicy), nil
}
