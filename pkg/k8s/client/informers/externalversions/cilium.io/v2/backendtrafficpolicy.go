...
type BackendTrafficPolicyInformer interface {
	...
	Lister() v2.BackendTrafficPolicyLister
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.CiliumV2().BackendTrafficPolicies(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.CiliumV2().BackendTrafficPolicies(namespace).Watch(context.TODO(), options)
			},
		},
		&ciliumiov2.BackendTrafficPolicy{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *backendTrafficPolicyInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&ciliumiov2.BackendTrafficPolicy{}, f.defaultInformer)
}

func (f *backendTrafficPolicyInformer) Lister() v2.BackendTrafficPolicyLister {
	return v2.NewBackendTrafficPolicyLister(f.Informer().GetIndexer())
}
